...
function App() {
  ...
  const handleAdd = () => {
    if (newTodo.trim()) {
      setTodo([{ id: Date.now(), task: newTodo, completed: false }, ...todo]);
      setNewTodo("");
    }
  };
  ...
  useEffect(() => {
    // Save to-dos and theme to local storage
    try {
  ...
  useEffect(() => {
    // Load to-dos and theme from local storage
    try {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-20 py-14 bg-no-repeat">
        <div className="flex flex-col gap-8 w-full lg:w-[35rem]">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setTheme(!theme)} aria-label="Toggle theme">
              <img src={theme ? sun : moon} alt="Theme toggle icon" />
            </button>
          </div>
          ...
          <div className="flex items-center">
            <button
              onClick={() => handleAdd()}
              className={`border-2 ${
                theme ? "border-Very-Dark-Grayish-Blue" : "border-Very-Dark-Grayish-Blue-1"
            ...
            <input
              ...
              className={`${
                theme ? "bg-Very-Dark-Desaturated-Blue text-Light-Grayish-Blue" : "bg-Very-Light-Gray text-Very-Dark-Desaturated-Blue"
            ...
            <ul className="flex flex-col divide-y-2 rounded-t-lg">
              {filtered.map((t, index) => (
                <Reorder.Item key={t.id} value={t}>
                  <li className="py-3 flex items-center justify-between text-sm md:text-lg px-3">
                    <div className="flex items-center gap-5">
                      <button
                        onClick={() => handleCheck(index)}
                        className={`border-2 ${t.completed ? "checkBox" : ""} w-6 h-6 ${
                          theme ? "border-Very-Dark-Grayish-Blue" : "border-Very-Dark-Grayish-Blue-1"
                        } rounded-full flex items-center justify-center`}
                      >
                        <img
                          className={`${t.completed ? "opacity-100" : "opacity-0"}`}
                          src={check}
                          alt="Check icon"
                        />
                      </button>
                      <p className={`${theme ? "" : "text-Very-Dark-Grayish-Blue-1"} ${t.completed ? "line-through" : ""}`}>
                        {t.task}
                      </p>
                    </div>
